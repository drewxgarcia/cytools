...
# LRU with a hard entry cap, same policy as `dataset._BoundedCache`: a plain
# dict here would be unbounded process-level retention. Values are bools.
_SOLID_CACHE: OrderedDict[tuple, bool] = OrderedDict()
_SOLID_CACHE_MAX = 8192
    ...
    **Example:**
        ...
        if len(data):
            ...
            if check or t in (fmpz, np.float64):
                ...
                if t == np.int64:
                    ...
                    if False in mask:
                        warnings.warn(
                            "0 gcd found (row of zeros)... Skipping it!", stacklevel=2
                        )
    ...
    def __repr__(self):
        ...
        **Example:**
        ...
        if self._rays is not None:
            return (
                f"A {self.dimension()}-dimensional rational polyhedral cone in "
    ...
    def __eq__(self, other):
        ...
        **Example:**
        ...
        if self.dual_cone().is_pointed() ^ other.dual_cone().is_pointed():
            return False
        if self.dual_cone().is_pointed() and other.dual_cone().is_pointed():
            return sorted(self.dual_cone().extremal_rays().tolist()) == sorted(
                other.dual_cone().extremal_rays().tolist()
            )
    ...
    def dimension(self):
        ...
        **Example:**
        ...
        if self._rays is not None:
            ...
            if self.is_solid():
                self._dim = self.ambient_dimension()
            else:
    ...
    def extremal_rays(
        ...
        **Arguments:**
        ...
        - `method`: The backend used to prune the rays. One of
            ...
            package, else falls back to "legacy"), "legacy", or "nnls".
        ...
        **Example:**
        ...
        if rays.shape[0] == 0:
            ext_rays = rays.reshape(0, self.ambient_dimension())
            self._ext_rays[minimal] = ext_rays
            return ext_rays
    ...
    def extremal_hyperplanes(
        ...
        **Returns:**
        ...
        """
        return self.dual_cone().extremal_rays(
    ...
    def face_lattice(
        ...
        **Returns:**
        ...
        if dim > 20:
            warnings.warn(
                "Getting the face lattice for high-dim cones is expensive", stacklevel=2
            )
        ...
        if codim == dim:
            if self.is_pointed():
                ...
                identity = np.eye(self.ambient_dimension(), dtype=int)
                return (Cone(hyperplanes=np.vstack([identity, -identity])),)
        ...
        if self.is_solid():
            ...
            can_saturate = np.array(
                [h for h in H if not self.dual_cone().contains(-h)],
                dtype=int,
            )
        ...
        for h in can_saturate:
            ray_inds = frozenset(
                ...
                if sum(int(a) * int(b) for a, b in zip(h, r, strict=True)) == 0
            )
            if ray_inds:
        ...
        if self.is_pointed():
            identity = np.eye(self.ambient_dimension(), dtype=int)
            face_lattice.append((Cone(hyperplanes=np.vstack([identity, -identity])),))
        ...
        **Returns:**
        ...
        if self.is_simplicial():
            ...
            if dim == 1:
                identity = np.eye(self.ambient_dimension(), dtype=int)
                return [
                    Cone(hyperplanes=np.vstack([identity, -identity])),
                ]
            if dim == 0:
                return []
        ...
        if self.is_solid():
            ...
            can_saturate = [h for h in H if not self.dual_cone().contains(-h)]
    ...
    def tip_of_stretched_cone(
        ...
        **Example:**
        ...
        problem = {"dim": self.ambient_dimension(), "rows": len(hp)}
        ...
        if solution is None:
            if show_hints and CERTIFIES_INFEASIBLE not in engine.provides:
                ...
                if (self.ambient_dimension() >= 25) and (engine.name != "mosek"):
                    print(
                        f"-) given the high dimension, {self.ambient_dimension()},",
                        end=" ",
                    )
                    print(
    ...
    def find_grading_vector(
        ...
        **Example:**
        ...
        return self.dual_cone().find_interior_point(integral=True, backend=backend)
    ...
    def find_lattice_points(
        ...
        **Example:**
        ...
        if fast_mode and (min_points is not None) and (self.ambient_dimension() <= 10):
            return np.array(
        ...
        def make_lattice_model():
            ...
            # define constraints
            for h, cc in zip(hp, c_vals, strict=True):
        ...
        out = [sorted(pts[degs == deg].tolist()) for deg in sorted(set(degs))]
    ...
    def is_solid(self, backend: InteriorPointBackend | str | None = None) -> bool:
        ...
        **Example:**
        ...
        # Solidity of {x : Ax >= 0} is a pure function of A, and landscape work
        # asks it about the same A over and over: an `ntfe` sweep of 120
        # polytopes at h11=7 reaches this LP 1,172 times over just 44 distinct
        # hyperplane matrices, because the 2-faces being checked for regularity
        # come in only ~50 shapes. Memoizing on the matrix bytes removes 96% of
        # the solver calls. The cached value is a bool, so there is nothing to
        # copy and nothing a caller can corrupt.
        _solid_key = None
        if self._hyperplanes is not None and np.size(self._hyperplanes) <= 4096:
            # Small systems are the 2-face regularity checks, which recur; a
            # 4d polytope's own secondary cone is far larger and unique to it,
            # so it is left to go straight to the solver.
            _h = np.ascontiguousarray(np.asarray(self._hyperplanes, dtype=np.int64))
            _solid_key = (_h.shape, _h.tobytes(), str(backend))
            _cached = _SOLID_CACHE.get(_solid_key)
            if _cached is not None:
                _SOLID_CACHE.move_to_end(_solid_key)
                self._is_solid = _cached
                return self._is_solid
        ...
        if _solid_key is not None:
            _SOLID_CACHE[_solid_key] = self._is_solid
            while len(_SOLID_CACHE) > _SOLID_CACHE_MAX:
    ...
    def is_pointed(self) -> bool:
        ...
        **Example:**
        ...
        if self._is_pointed is None:
            self._is_pointed = self.dual_cone().is_solid()
    ...
    def is_simplicial(self):
        ...
        **Description:**
        ...
        N.B.: if c is solid, then c is simplicial <=> c.dual_cone() is simplicial.
        ...
        **Example:**
        ...
        if (self._rays is None) and (self.is_solid()):
            self._is_simplicial = len(self.extremal_hyperplanes()) == self.dimension()
        else:
            self._is_simplicial = len(self.extremal_rays()) == self.dimension()
    ...
    def is_degenerate(
        ...
        **Returns:**
        ...
        xtest = np.ones(self.ambient_dimension(), dtype=int)
        dists = H @ xtest
        z = dists == 0
        if sum(z) >= self.ambient_dimension() + 1:
    ...
    def intersection(self, other):
        ...
        **Example:**
        ...
        for c in others:
            ...
            if c.ambient_dimension() != self.ambient_dimension():
...
def _is_degenerate(
    ...
    **Returns:**
    ...
    for i, v in enumerate(H):
        dist = sum(_x * _v for _x, _v in zip(x, v, strict=True))
    ...
    if status in (cp_model.FEASIBLE, cp_model.OPTIMAL):
        ...
        if verbosity >= 1:
            warnings.warn("Solver returned status INFEASIBLE.", stacklevel=2)
        ...
        warnings.warn(f"Solver returned status {status_list[status]}.", stacklevel=2)
