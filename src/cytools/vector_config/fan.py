    ...
    def restricted_simps(
        ...
        **Returns:**
        ...
        for face in p.faces(to_dim):
            ...
            face_simps_reduced: list[list[int]] = []
            ...
            if as_face_inds:
                ...
                # Normalize to sorted lists. The reduction above yields
                # frozensets, but the `padded` step below indexes and
                # concatenates, which a frozenset does not support.
            ...
            # save it!
            face_simps_reduced = sorted([sorted(simp) for simp in face_simps_reduced])
        ...
        for r in range(dim - 1, 0, -1):  # dim-1, dim-2, ..., 1
            ...
            for face, neighbs in neighbors[r].items():
                ...
                for duplicates in itertools.combinations_with_replacement(
                    ...
                    # of what follows
                    known_vals = np.array(
                        [self._kappa[insert_sorted(prefactor, i)] for i in neighbs]
                    ).reshape(-1, 1)
