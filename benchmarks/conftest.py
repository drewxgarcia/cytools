...
def bulk_polys():
    """
    Polytopes drawn from the 13-17 vertex-count files, which together hold
    65.9% of all 438M KS polytopes (median h11 ≈ 30-45).

    This is the most representative single-tier sample for pipeline benchmarks:
    it reflects where real KS scan workloads spend the vast majority of their
    time.  Default 20 polytopes (4 per vertex-count file).

    Override sample size via CYTOOLS_BENCH_N_BULK env var.
    """
...
def _load_cy_polys_range(h11_min: int, h11_max: int, n: int, seed: int = 42):
    """Return n polytopes with h11 in [h11_min, h11_max], sampled evenly across
    the range.  Used to build representative samples at the DB median complexity
    rather than the extreme low tail.
    """
    import math
    span = h11_max - h11_min + 1
    n_per_h11 = max(1, math.ceil(n / span))
    records = []
...
@pytest.fixture(scope="module")
def cy_polys_median():
    """20 polytopes with h11 in [20, 35] — the actual median range of the KS DB.

    The KS database median h11 is ~30-45 depending on vertex count.  The
    cy_polys fixture (h11 <= 4) samples the extreme low tail and benchmarks
    unrealistically fast cases.  This fixture gives the typical-complexity
    population that reflects real scan workloads.

    These polytopes take ~20-40ms each for the full pipeline vs ~2-5ms for
    the h11 <= 4 population.

    Override sample size via CYTOOLS_BENCH_N_CY_MEDIAN env var.
    """
    n = int(os.environ.get("CYTOOLS_BENCH_N_CY_MEDIAN", "20"))
    return _load_cy_polys_range(h11_min=20, h11_max=35, n=n)
